...
void get_info(char *argv[],int *typep,char *classp)
{
  ...
  if (!(strcmp(argv[1],"sp") != 0) || !(strcmp(argv[1],"SP") != 0)) 
     *typep = SP;
  else if (!(strcmp(argv[1],"bt") != 0) || !(strcmp(argv[1],"BT") != 0)) 
     *typep = BT;
  else if (!(strcmp(argv[1],"ft") != 0) || !(strcmp(argv[1],"FT") != 0)) 
     *typep = FT;
  else if (!(strcmp(argv[1],"lu") != 0) || !(strcmp(argv[1],"LU") != 0)) 
     *typep = LU;
  else if (!(strcmp(argv[1],"mg") != 0) || !(strcmp(argv[1],"MG") != 0)) 
     *typep = MG;
  else if (!(strcmp(argv[1],"is") != 0) || !(strcmp(argv[1],"IS") != 0)) 
     *typep = IS;
  else if (!(strcmp(argv[1],"ep") != 0) || !(strcmp(argv[1],"EP") != 0)) 
     *typep = EP;
  else if (!(strcmp(argv[1],"cg") != 0) || !(strcmp(argv[1],"CG") != 0)) 
     *typep = CG;
  else {
...
void check_info(int type,char classc)
{
  int tmplog;
/* check classc */
  if ((((((classc != 'S') && (classc != 'A')) && (classc != 'B')) && (classc != 'R')) && (classc != 'W')) && (classc != 'C')) {
...
void write_is_info(FILE *fp,char classc)
{
  ...
  int m2;
  int m3;
  if (((((classc != 'S') && (classc != 'W')) && (classc != 'A')) && (classc != 'B')) && (classc != 'C')) {
    printf("setparams: Internal error: invalid classc type %c\n",classc);
    exit(1);
  }
}
...
void write_compiler_info(int type,FILE *fp)
{
  ...
  char f_inc[400UL];
  char fflags[400UL];
  char flinkflags[400UL];
  char compiletime[400UL];
  char randfile[400UL];
  char cc[400UL];
  char cflags[400UL];
  char clink[400UL];
  char clinkflags[400UL];
  char c_lib[400UL];
  char c_inc[400UL];
  struct tm *tmp;
  time_t t;
  deffile = fopen("../config/make.def","r");
  if (deffile == ((FILE *)((void *)0))) {
  ...
  strcpy(c_lib,"(none)");
  strcpy(c_inc,"(none)");
  while(fgets(line,400,deffile) != ((char *)((void *)0))){
    if (( *line) == '#') 
      continue; 
/* yes, this is inefficient. but it's simple! */
  ...
  time(&t);
  tmp = localtime((&t));
  strftime(compiletime,((size_t )400),"%d %b %Y",tmp);
  switch(type){
    case 0:
{
    }
    ...
    case 7:
{
      ...
      put_def_string(fp,"NPBVERSION","2.3");
...
/* if *line is not a space, actual label is longer than test label */
  if (!((( *__ctype_b_loc())[(int )( *line)] & ((unsigned short )_ISspace)) != 0) && (( *line) != '=')) 
    return ;
/* skip over white space */
  while((( *__ctype_b_loc())[(int )( *line)] & ((unsigned short )_ISspace)) != 0)
    line++;
/* next char should be '=' */
  if (( *line) != '=') 
    return ;
...
/* if *line is not a space, first word is not "include" */
  if (!((( *__ctype_b_loc())[(int )( *line)] & ((unsigned short )_ISspace)) != 0)) 
    return 0;
/* skip over white space */
  while((( *__ctype_b_loc())[(int )( *(++line))] & ((unsigned short )_ISspace)) != 0);
/* if EOS, nothing was specified */
...
/* next keyword should be name of include file in *filename */
  ...
  if ((( *filename) != 0) || (((( *line) != 32) && (( *line) != 0)) && (( *line) != 10))) 
    return 0;
  else 
    return 1;
}
